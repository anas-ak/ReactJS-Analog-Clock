...
export default class Clock extends React.Component {
    ...
    drawFace(ctx, radius) {
        ...
        ctx.fillStyle = "white"; // clock face
        ctx.fill();

        const grad = ctx.createRadialGradient(0, 0, radius * 0.95, 0, 0, radius * 1.05);

        grad.addColorStop(0, "#333");
        grad.addColorStop(0.5, "black");    // outer grid
    ...
    drawNumbers(ctx, radius) {
        ...
        const fontBig = radius * 0.15 + "px monospace";
        const fontSmall = radius * 0.075 + "px monospace";
        ...
        for(num = 1; num < 13; num++) {
            ...
            ctx.font = fontBig;
            ctx.fillStyle = "black";    // 12-hour format
    ...
    drawTime(ctx, radius) {
        ...
        this.drawHand(ctx, hour, radius * 0.5, radius * 0.05, "green");

        // display minute-hand
        minute = (minute * Math.PI / 30) + (second * Math.PI / (30 * 60));
        this.drawHand(ctx, minute, radius * 0.8, radius * 0.05, "blue");
...
//     timeFormat: "24hour",   // {standard | 24hour}
//     hourFormat: "standard"
// };
